...
func (self *shakerChannel) Start(fadeIn TicksDuration) {
	if self.fadeIn == fadeIn && self.IsFadingIn() { return }
...
func (self *shakerChannel) End(fadeOut TicksDuration) {
	...
	if self.fadeOut == fadeOut && self.IsFadingOut() { return }
	activity := self.Activity()
...
func (self *shakerChannel) IsShaking() bool {
	if self.elapsed == 0 {
		...
		if self.elapsed < self.duration { return true }
...
func (self *shakerChannel) Activity() float64 {
	...
	if self.elapsed < self.fadeIn {
		...
		if elapsed <= self.duration { return 1.0 } // shake in progress
		elapsed -= self.duration
		if elapsed >= self.fadeOut { return 0.0 }
