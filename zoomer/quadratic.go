...
func (self *Quadratic) SetAcceleration(acceleration float64) {
	if acceleration < 0.01 {
		panic("acceleration can't be < 0.01")
	}
...
func (self *Quadratic) Update(currentZoom, targetZoom float64) float64 {
	...
	predicted := (self.speed * self.speed) / (2.0 * ebimath.Abs(self.acceleration))
	target := ebimath.Abs(distance)
	...
	if ebimath.Abs(change) < 0.001 && ebimath.Abs(distance) < 0.001 {
		self.speed = 0.0
		return distance
	}
	return change
